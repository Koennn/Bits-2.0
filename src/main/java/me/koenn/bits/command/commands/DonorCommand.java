...
public class DonorCommand implements ICommand {
    ...
    public String execute(CPlayer cPlayer, String[] args) {
        ...
        if (arg.equalsIgnoreCase("set")) {
            return this.setDonor(player);
        } else if (arg.equalsIgnoreCase("del")) {
            return this.delDonor(player);
        } else {
            return Ref.NEGATIVE_UNKNOWN_ARG.replace("{arg}", arg);
        }
    }

    private String setDonor(CPlayer cPlayer) {
        if (cPlayer.isDonor()) {
            return Ref.NEGATIVE_ALREADY_DONOR;
        }

        cPlayer.set("donor", true);
        PermissionHelper.reloadPermissions(cPlayer);

        return Ref.POSITIVE_SET_DONOR.replace("{player}", cPlayer.getName());
    }

    private String delDonor(CPlayer cPlayer) {
        if (!cPlayer.isDonor()) {
            return Ref.NEGATIVE_NOT_DONOR;
        }

        cPlayer.set("donor", false);
        PermissionHelper.reloadPermissions(cPlayer);

        return Ref.POSITIVE_DEL_DONOR.replace("{player}", cPlayer.getName());
    }
